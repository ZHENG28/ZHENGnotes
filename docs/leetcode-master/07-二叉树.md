...
:::info[如何根据两个顺序确定唯一的二叉树？]
以 **后序数组的最后一个元素** 作为切割点
1. 切割中序数组
2. 按照中序左子数组的大小，切割后序数组
![Construct Binary Tree](./img/07-二叉树/18.1.construct-binary-tree.png)
:::
...
public:
    TreeNode* traversal(vector<int>& inorder, vector<int>& postorder) {
        ...
        // 3. 将根节点在中序数组中的位置作为切割点
        int delimiterIndex;
        for (delimiterIndex = 0; delimiterIndex < inorder.size(); delimiterIndex++) {
            if (inorder[delimiterIndex] == rootValue) break;
        }

        // 4. 切割中序数组（左闭右开），得到中序左右区间
        vector<int> leftInorder(inorder.begin(), inorder.begin() + delimiterIndex);
        vector<int> rightInorder(inorder.begin() + delimiterIndex + 1, inorder.end());
        // 5. 切割后序数组（左闭右开），得到后序左右区间
...
```
1. 相关题目：[LC105](https://leetcode.cn/problems/construct-binary-tree-from-preorder-and-inorder-traversal/description/)
```cpp showLineNumbers
class Solution {
public:
    // 用数组下标索引来分割，左闭右开[leftIndex, rightIndex)
    TreeNode* traversal(vector<int>& inorder, int inorderLeftIndex, int inorderRightIndex,
                        vector<int>& preorder, int preorderLeftIndex, int preorderRightIndex) {
        // 1. 如果数组大小为零，则为空节点
        if (preorderLeftIndex == preorderRightIndex) return nullptr;

        // 2. 如果不为空，则取后序数组的最后一个元素作为根节点
        int rootValue = preorder[preorderLeftIndex];
        TreeNode* root = new TreeNode(rootValue);

        // 2.1. 如果为叶子节点，则直接返回
        if (preorderRightIndex - preorderLeftIndex == 1) return root;

        // 3. 将根节点在中序数组中的位置作为切割点
        int delimiterIndex;
        for (delimiterIndex = inorderLeftIndex; delimiterIndex < inorderRightIndex; delimiterIndex++) {
            if (inorder[delimiterIndex] == rootValue) break;
        }

        // 4 & 5 & 6. 切割中序数组 & 切割后序数组 & 递归处理左右区间
        preorderLeftIndex++;
        root->left = traversal(inorder, inorderLeftIndex, delimiterIndex,
                               preorder, preorderLeftIndex, preorderLeftIndex + (delimiterIndex - inorderLeftIndex));
        root->right = traversal(inorder, delimiterIndex + 1, inorderRightIndex,
                                preorder, preorderLeftIndex + (delimiterIndex - inorderLeftIndex), preorderRightIndex);
...
:::info[哪些顺序组合可以确定唯一的二叉树？]
1. 前序和中序
2. 后序和中序

**<font color="red">【根据中序数组来确定左右部分，才能分割前序或后序数组】</font>**
:::
...
> 【[LC617](https://leetcode.cn/problems/merge-two-binary-trees/description/)】给你两棵二叉树： root1 和 root2 。  
...
public:
    TreeNode* mergeTrees(TreeNode* root1, TreeNode* root2) {
        ...
        TreeNode* resultCur = new TreeNode(root1->val + root2->val);
        resultCur->left = mergeTrees(root1->left, root2->left);
        resultCur->right = mergeTrees(root1->right, root2->right);

        return resultCur;
    }
};
```
2. 迭代法：模拟层序遍历，但生成新树的代码较复杂（即直接将 `root1` 作为新节点），不建议
```cpp showLineNumbers
class Solution {
public:
    TreeNode* mergeTrees(TreeNode* root1, TreeNode* root2) {
...
> 【[LC700](https://leetcode.cn/problems/search-in-a-binary-search-tree/description/)】给定二叉搜索树（BST）的根节点 root 和一个整数值 val。你需要在 BST 中找到节点值等于 val 的节点。 返回以该节点为根的子树。 如果节点不存在，则返回 null 。
...
```
3. 迭代法：二叉搜索树节点的有序性确定了搜索的方向 &rarr; 不需要 **回溯**
```cpp showLineNumbers
class Solution {
public:
    TreeNode* searchBST(TreeNode* root, int val) {
...
> 【[LC98](https://leetcode.cn/problems/validate-binary-search-tree/description/)】给你一个二叉树的根节点 root ，判断其是否是一个有效的二叉搜索树。有效 二叉搜索树定义如下：
> - 节点的左子树只包含 严格小于 当前节点的数。
> - 节点的右子树只包含 严格大于 当前节点的数。
> - 所有左子树和右子树自身必须也是二叉搜索树。

1. 自想解法：递归判断左右子节点与当前节点的大小 &rarr; 在多层节点上不可行

:::tip[如何判断是否是一个有效的二叉搜索树？]
1. 前提：**二叉搜索树通过中序遍历输出的是有序序列**
2. 验证二叉搜索树的有效，相当于判断一个序列是不是递增
:::
...
> 【[LC530](https://leetcode.cn/problems/minimum-absolute-difference-in-bst/description/)】给你一个二叉搜索树的根节点 root ，返回 树中任意两不同节点值之间的最小差值。差值是一个正数，其数值等于两值之差的绝对值。

1. 采用中序遍历的二叉搜索树 &rarr; 有序数组 &rarr; 在有序数组上求两数的最小差值
2. 递归法：
```cpp showLineNumbers
class Solution {
public:
...
> 【[LC501](https://leetcode.cn/problems/find-mode-in-binary-search-tree/description/)】给你一个含重复值的二叉搜索树（BST）的根节点 root ，找出并返回 BST 中的所有 众数（即，出现频率最高的元素）。如果树中有不止一个众数，可以按 任意顺序 返回。假定 BST 满足如下定义：
> - 结点左子树中所含节点的值 小于等于 当前节点的值
> - 结点右子树中所含节点的值 大于等于 当前节点的值
> - 左子树和右子树都是二叉搜索树

1. 二叉树求解思路：遍历树 &rarr; 用map统计频率 &rarr; 对频率排序 &rarr; 取高频元素集合
2. 递归法：
```cpp showLineNumbers
class Solution {
public:
    ...
    void traversal(TreeNode* cur) {
        ...
        // 2. 与当前最大出现次数比较 -> 是否放入结果集
        if (count == maxCount) result.push_back(cur->val);
        else if (count > maxCount) {
            // 发现存在出现次数更多的元素，需要记录其出现次数，并清空原结果集
            maxCount = count;
            result.clear();
            result.push_back(cur->val);
        }

        // 前一组已经比较完毕，将当前节点迭代为前一个节点
        pre = cur;
        traversal(cur->right);
    }
...
> 【[LC236](https://leetcode.cn/problems/lowest-common-ancestor-of-a-binary-tree/description/)】给定一个二叉树, 找到该树中两个指定节点的最近公共祖先。百度百科中最近公共祖先的定义为：“对于有根树 T 的两个节点 p、q，最近公共祖先表示为一个节点 x，满足 x 是 p、q 的祖先且 x 的深度尽可能大（一个节点也可以是它自己的祖先）。”

1. 寻找公共祖先 &rarr; 自底向上 &rarr; **回溯** &rarr; 后序遍历
    1. 找到一个节点，发现左子树出现结点p（q），右子树出现节点q（p） &rarr; 该节点是节点p和q的最近公共祖先
    2. 节点本身就是节点p（q），且拥有一个子节点q（p） &rarr; 该节点是节点p和q的最近公共祖先

:::tip[如果递归函数有返回值，如何区分搜索一条边，还是搜索整个树？]
- 搜索一条边：递归函数的返回值不为空时，立刻返回
```cpp showLineNumbers
if (traversal(root->left)) return result;
if (traversal(root->right)) return result;
```
- 搜索整个树：用变量接收返回值，并用变量处理中间节点的逻辑（即回溯），再返回
```cpp showLineNumbers
// 左
...
public:
    TreeNode* lowestCommonAncestor(TreeNode* root, TreeNode* p, TreeNode* q) {
        // 1. 如果当前节点为节点p或q，则直接返回
        if (root == p || root == q || root == nullptr) return root;

        TreeNode* left = lowestCommonAncestor(root->left, p, q);
        TreeNode* right = lowestCommonAncestor(root->right, p, q);
        // 2.1. 如果左右子节点返回值不为空，说明当前节点即为最近公共祖先
        if (left != nullptr && right != nullptr) return root;
        // 2.2. 如果左子节点为空且右子节点不为空，说明右子节点为最近公共祖先，直接返回
        else if (left == nullptr && right != nullptr) return right;
        // 2.3. 同上，说明左子节点为最近公共祖先，直接返回
...
:::tip[如何理解用递归法执行情况2？]
前提：节点p和q均存在于给定的二叉树中
判断到当前节点为节点p或q时，会直接返回当前节点；而同级节点会返回 `nullptr` &rarr; 最近公共祖先即为当前节点
:::
...
> 【[LC235](https://leetcode.cn/problems/lowest-common-ancestor-of-a-binary-search-tree/description/)】给定一个二叉搜索树, 找到该树中两个指定节点的最近公共祖先。百度百科中最近公共祖先的定义为：“对于有根树 T 的两个结点 p、q，最近公共祖先表示为一个结点 x，满足 x 是 p、q 的祖先且 x 的深度尽可能大（一个节点也可以是它自己的祖先）。”

1. 类似 [二叉树的最近公共祖先](#26-二叉树的最近公共祖先)，但可利用二叉搜索树的特性：
    - 公共祖先：有序 &rarr; 节点值一定 **在[p, q]或[q, p]区间内**
    - 最近公共祖先：有序 &rarr; **第一次遇到** 值在[p, q]或[q, p]区间内的节点
    ![lowest-common-ancestor](./img/07-二叉树/28.1.lowest-common-ancestor.png)
2. 自想解法：将二叉树的判断逻辑改为判断值区间
3. 递归法：
```cpp showLineNumbers
class Solution {
public:
...
> 【[LC701](https://leetcode.cn/problems/insert-into-a-binary-search-tree/description/)】给定二叉搜索树（BST）的根节点 root 和要插入树中的值 value ，将值插入二叉搜索树。 返回插入后二叉搜索树的根节点。 输入数据 保证 ，新值和原始二叉搜索树中的任意节点值都不同。注意，可能存在多种有效的插入方式，只要树在插入后仍保持为二叉搜索树即可。 你可以返回 任意有效的结果 。

1. 利用二叉搜索树的特性轻松AC，且此题不需要重构二叉树

---

## 30. 删除二叉搜索树中的节点

> 【[LC450](https://leetcode.cn/problems/delete-node-in-a-bst/description/)】给定一个二叉搜索树的根节点 root 和一个值 key，删除二叉搜索树中的 key 对应的节点，并保证二叉搜索树的性质不变。返回二叉搜索树（有可能被更新）的根节点的引用。一般来说，删除节点可分为两个步骤：
> 1. 首先找到需要删除的节点；
> 2. 如果找到了，删除它。

1. 二叉搜索树中删除节点的情况：
    - 没找到删除的节点：遍历到空节点则直接返回
    - 找到删除的节点：
        1. 删除节点的左右子节点都为空：直接删除节点，返回空指针为新的根节点
        2. 删除节点的左子节点为空，右子节点不为空：删除节点，右子节点补位，返回右子节点为根节点
        3. 删除节点的右子节点为空，左子节点不为空：删除节点，左子节点补位，返回左子节点为根节点
        4. 删除节点的左右子节点都不为空：将左子节点放到右子节点的最左边子节点的左子节点位置上，返回右子节点为根节点
            ![delete-node](./img/07-二叉树/30.1.delete-node.gif)
2. 递归法：
```cpp showLineNumbers
class Solution {
public:
    TreeNode* deleteNode(TreeNode* root, int key) {
        ...
        if (root->val > key) root->left = deleteNode(root->left, key);
            ...
            // 2.删除节点的左子节点为空，右子节点不为空：删除节点，右子节点补位，返回右子节点为根节点
            else if (root->left == nullptr && root->right != nullptr) root = root->right;
            // 3.删除节点的右子节点为空，左子节点不为空：删除节点，左子节点补位，返回左子节点为根节点
...
> 【[LC669](https://leetcode.cn/problems/trim-a-binary-search-tree/description/)】给你二叉搜索树的根节点 root ，同时给定最小边界low 和最大边界 high。通过修剪二叉搜索树，使得所有节点的值在[low, high]中。修剪树 不应该 改变保留在树中的元素的相对结构 (即，如果没有被移除，原有的父代子代关系都应当保留)。 可以证明，存在 唯一的答案 。所以结果应当返回修剪好的二叉搜索树的新的根节点。注意，根节点可能会根据给定的边界发生改变。
...
public:
    TreeNode* trimBST(TreeNode* root, int low, int high) {
        ...
        // 节点值不在区间内：左子节点待删除（即左子节点的值小于low），则左子树不考虑（即递归右子树） -> 直接返回找到的节点
        if (root->val < low) return trimBST(root->right, low, high);
        // 节点值不在区间内：右子节点要删除（即右子节点的值大于high），则右子树不考虑（即递归左子树） -> 直接返回找到的节点
        else if (root->val > high) return trimBST(root->left, low, high);
        else {
            // 判断条件：root->val >= low || root->val <= high
            // 节点值在区间内：遍历左右子树
...
public:
    TreeNode* trimBST(TreeNode* root, int low, int high) {
        ...
        // 此时root已经在[low, high] 范围内，处理左子节点值小于low的情况
        TreeNode* cur = root;
        while (cur != nullptr) {
        ...
        // 此时root已经在[low, high] 范围内，处理右子节点值大于high的情况
        cur = root;
        while (cur != nullptr) {
...
> 【[LC108](https://leetcode.cn/problems/convert-sorted-array-to-binary-search-tree/description/)】给你一个整数数组 nums ，其中元素已经按 升序 排列，请你将其转换为一棵 平衡 二叉搜索树。

1. 构造二叉树：本质就是寻找分割点，分割点作为当前节点，然后递归左区间和右区间 &rarr; 有序数组构造二叉搜索树，分割点就是数组中间位置的节点
2. 递归法：
```cpp showLineNumbers
class Solution {
public:
...
```
3. 迭代法：下标采用队列存储 &larr; 需要遍历左右两个区间
```cpp showLineNumbers
class Solution {
public:
    TreeNode* sortedArrayToBST(vector<int>& nums) {
...
> 【[LC538](https://leetcode.cn/problems/convert-bst-to-greater-tree/description/)】给出二叉 搜索 树的根节点，该树的节点值各不相同，请你将其转换为累加树（Greater Sum Tree），使每个节点 node 的新值等于原树中大于或等于 node.val 的值之和。提醒一下，二叉搜索树满足下列约束条件：
> - 节点的左子树仅包含键 小于 节点键的节点。
> - 节点的右子树仅包含键 大于 节点键的节点。
> - 左右子树也必须是二叉搜索树。

1. 利用二叉搜索树的特性轻松AC，并保持右中左的顺序
2. 递归法：
```cpp showLineNumbers
class Solution {
public:
...
1. 分类：
    1. 二叉树的理论基础：
        1. [1-二叉树理论基础](#1-二叉树理论基础)：二叉树的种类、存储方式、遍历方式、定义方式
    2. 二叉树的遍历方式：
        1. 深度优先遍历：[2-二叉树的递归遍历](#2-二叉树的递归遍历)、[3-二叉树的迭代遍历](#3-二叉树的迭代遍历)、[4-二叉树的统一迭代法](#4-二叉树的统一迭代法)
        2. 广度优先遍历：[5-二叉树的层序遍历](#5-二叉树的层序遍历)
    3. 求二叉树的属性：
        1. [二叉树：是否对称](#8-对称二叉树)：
            - 递归：后序，比较的是根节点的左子树与右子树是不是相互翻转
            - 迭代：使用队列/栈将两个节点顺序放入容器中进行比较
        2. [二叉树：求最大深度](#9-二叉树的最大深度)：
            - 递归：后序，求根节点最大高度就是最大深度，通过递归函数的返回值做计算树的高度
            - 迭代：层序遍历
        3. [二叉树：求最小深度](#10-二叉树的最小深度)：
            - 递归：后序，求根节点最小高度就是最小深度，注意最小深度的定义
            - 迭代：层序遍历
        4. [二叉树：求有多少个节点](#11-完全二叉树的节点个数)：
            - 递归：后序，通过递归函数的返回值计算节点数量
            - 迭代：层序遍历
        5. [二叉树：是否平衡](#12-平衡二叉树)：
            - 递归：后序，注意后序求高度和前序求深度，递归过程判断高度差
            - 迭代：效率很低，不推荐
        6. [二叉树：找所有路径](#13-二叉树的所有路径)：
            - 递归：前序，方便让父节点指向子节点，涉及回溯处理根节点到叶子的所有路径
            - 迭代：一个栈模拟递归，一个栈来存放对应的遍历路径
        7. [二叉树：求左叶子之和](#15-左叶子之和)：
            - 递归：后序，必须三层约束条件，才能判断是否是左叶子。
            - 迭代：直接模拟后序遍历
        8. [二叉树：求左下角的值](#16-找树左下角的值)：
    ...
    4. 二叉树的修改与构造：
        ...
        4. [合并两个二叉树](#21-合并二叉树)：
            ...
            - 迭代：使用队列，类似层序遍历
    5. 求二叉搜索树的属性：
        1. [二叉搜索树中的搜索](#22-二叉搜索树中的搜索)：
            - 递归：二叉搜索树的递归是有方向的
            - 迭代：因为有方向，所以迭代法很简单
        2. [是不是二叉搜索树](#23-验证二叉搜索树)：
            - 递归：中序，相当于变成了判断一个序列是不是递增的
            - 迭代：模拟中序，逻辑相同
        3. [求二叉搜索树的最小绝对差](#24-二叉搜索树的最小绝对差)：
            - 递归：中序，双指针操作
            - 迭代：模拟中序，逻辑相同
        4. [求二叉搜索树的众数](#25-二叉搜索树中的众数)：
            - 递归：中序，清空结果集的技巧，遍历一遍便可求众数集合
        5. [二叉搜索树转成累加树](#33-把二叉搜索树转换为累加树)：
            - 递归：中序，双指针操作累加
            - 迭代：模拟中序，逻辑相同
    6. 二叉树公共祖先问题：
        1. [二叉树的公共祖先问题](#26-二叉树的最近公共祖先)：
            - 递归：后序，回溯，找到左子树出现目标值，右子树节点目标值的节点。
            - 迭代：不适合模拟回溯
        2. [二叉搜索树的公共祖先问题](#28-二叉搜索树的最近公共祖先)：
            - 递归：顺序无所谓，如果节点的数值在目标区间就是最近公共祖先
            - 迭代：按序遍历
    7. 二叉搜索树的修改与构造：
        1. [二叉搜索树中的插入操作](#29-二叉搜索树中的插入操作)：
            - 递归：顺序无所谓，通过递归函数返回值添加节点
            - 迭代：按序遍历，需要记录插入父节点，这样才能做插入操作
        2. [二叉搜索树中的删除操作](#30-删除二叉搜索树中的节点)：
            - 递归：前序，想清楚删除非叶子节点的情况
            - 迭代：有序遍历，较复杂
        3. [修剪二叉搜索树](#31-修剪二叉搜索树)：
            - 递归：前序，通过递归函数返回值删除节点
            - 迭代：有序遍历，较复杂
        4. [构造二叉搜索树](#32-将有序数组转换为二叉搜索树)：
            - 递归：前序，数组中间节点分割
            - 迭代：较复杂，通过三个队列来模拟

:::danger[二叉树题目选择什么遍历顺序？]
1. 构造普通二叉树/二叉搜索树：**前序**（先构造中节点）
2. 求普通二叉树的属性：**后序**（通过递归函数的返回值做计算）
3. 求二叉搜索树的属性：**中序**（有序性）
:::

2. ![summary](./img/07-二叉树/34.1.summary.png)