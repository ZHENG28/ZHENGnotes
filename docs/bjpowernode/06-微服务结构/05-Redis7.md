...
0. key常用操作命令：
    ...
    # 查看指定key的剩余生存时间ttl（time to live）
    # key没有设置生存时间：-1；key不存在：-2
    ttl key

    # 设置key的最大生存时间（设置成功：1；其他：0）
    expire key seconds

    # 查看指定key的数据类型（不存在：none）
    type key
...
1. **<font color="red">字符串类型string</font>**：`单key:单value`
    ...
    - 常用操作命令：
        ...
        # 获取字符串key中从startIndex到endIndex（闭区间）的字符组成的子字符串
        # 下标自左至右，从0开始，依次往后；负下标自右至左，从-1开始，依次往前
...
2. **<font color="red">列表类型list</font>**：`单key:多有序value`
    ...
    - 常用操作命令：
        ...
        # 从指定列表中，从表头移除并且返回指定元素
        lpop key
        # 从指定列表中，从表尾移除并且返回指定元素
        rpop key
        ...
        # 截取指定列表中指定下标区间的元素组成新的列表且赋值给key
        ltrim key startIndex endIndex

        # 将指定列表中指定下标的元素设置为指定值
        lset key index value

        # 将value插入到指定列表中位于pivot元素之前或之后的位置
...
5. **<font color="red">有序集合类型zset（sorted set）</font>**：`单key:多有序value`
    ...
    - 常用操作命令：
        ...
        zrange/zrevrange key startIndex endIndex [withscores]
        ...
        # 获取指定有序集合中指定元素的排名（分数升序/降序reverse）
