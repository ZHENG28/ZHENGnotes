...
1. 基于动态代理，可以使用JDK、CGLIB两种代理方式 &rarr; 规范化动态代理
    ...
    - CGLIB动态代理：使用第三方的工具库创建代理对象；要求目标类必须继承目标类，创建子类（即代理对象）
...
### 4.4 集成第三方库
1. 集成MyBatis：MyBatis与spring集成时，事务会**自动提交**
    1. 声明数据源DataSource
...
3. 执行流程：
    ```mermaid
    graph TB
    发起请求 --> HandlerMapper[DispatcherServlet接受请求，将请求转交给处理器映射器] --> 将HandlerExecutionChain中的处理器对象交给处理器适配器对象 --> 将ModelAndView交给视图解析器对象 --> View[获取到View对象，调用View类的方法] --> Model[将Model数据放入到request作用域，并执行视图转发] --> End[请求结束，返回响应]
    ```
4. 注解式开发：
